...
use crate::memo::{Memo, TxType, TxSelector};
...
pub fn parse_calldata(
    bytes: Vec<u8>,
    rpc: Option<String>,
) -> Result<Calldata, MemoParserError> {
    ...
    if selector == TxSelector::Transact && bytes.len() < 644 {
        return Err(MemoParserError::ParseError(format!(
            "Incorrect calldata length! It must be at least 644 bytes for {} selector", selector
        )));
    }
...
mod tests {
    ...
    fn test_parse_calldata_1() {
        ...
        assert_eq!(parsed_calldata.tx_type, TxType::Deposit);
    ...
    fn test_parse_calldata_2() {
        ...
        assert_eq!(parsed_calldata.tx_type, TxType::Transfer);
